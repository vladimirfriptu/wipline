...
export function renderBoard(summaries, cursor, width) {
  ...
  summaries.forEach((summary, index) => {
    ...
    // "in progress" from "not started". Several can run at once, so say how many
    // rather than silently showing the first and hiding the rest.
    const wips = summary.wip ?? [];
    const extra = wips.length > 1 ? ` +${wips.length - 1}` : '';
    const next = wips.length > 0 ? `${GLYPHS.wip} ${wips[0]}${extra}` : (summary.next ?? 'done');
